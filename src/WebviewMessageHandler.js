...
export const MessageConverter = (action) => {
  switch(action.type) {
    ...
    case actions.insertImageIntoGrid:
      return `zss_editor.insertImageIntoGrid(${action.data})`;
    case actions.updateGridView:
      return `zss_editor.updateGridView(${action.data})`;
    case `${actions.updateImageWithUrl}`:
      return `zss_editor.updateImageWithUrl(${action.data});`;
    case `${actions.removeDimFilter}`:
      return `zss_editor.removeDimFilter(${action.data});`;
    ...
    case actions.createGridImageGroup:
      return `zss_editor.createGridImageGroup(${action.data})`;
    case actions.removeImageWithId:
      return `zss_editor.removeImageWithId('${action.data}')`;
  }
};
